myFDRTOOL <- function (x, statistic = c("normal", "correlation", "pvalue"), 
              plot = TRUE, color.figure = TRUE, verbose = TRUE,  
              cutoff.method = c("fndr", "pct0", "locfdr"), pct0 = 0.75) 
{
  ### This code is a copy of the function fdrtool() from the R package 
  ### 'fdrtool' (Strimmer, 2008). A modification was made to the plotting part. 
  ###
  ...
  if (file.exists('nullmodel.R')) 
    source('nullmodel.R')
  if (file.exists('ecdf.pval.R'))
    source('ecdf.pval.R')
  ...
  if (plot) {
    ...
    if (statistic == "normal") {
      ll = list(main= substitute(sigma == a *","~ eta[0] == b, 
                                 list(a=round(scale.param,2),b=round(eta0,3) )),
                xlab= expression(z == atanh(sqrt(rho^2))))
                #xlab= expression(paste('abs(corr(',y[j],',',x[i],'|',x[rest],'))',sep='')))
    } else {
      ll = list(main= substitute(kappa == a *","~ eta[0] == b, 
                                 list(a=round(scale.param,2),b=round(eta0,3) )),
                xlab= expression(sqrt(rho^2)))
                #xlab= expression(paste('abs(corr(',y[j],',',x[i],'|',x[rest],'))',sep='')))      
    }
